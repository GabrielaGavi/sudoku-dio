...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Validação dos argumentos recebidos
        try {
            ...
            System.out.println("Argumentos inválidos! O jogo não poderá iniciar.");
            return;
        }
    ...
    private static void mostrarMenu() {
        ...
        System.out.println("9 - Pedir dicas");
    }
    ...
    private static void startGame(final Map<String, String> positions) {
        ...
        for (int i = 0; i < BOARD_LIMIT; i++) {
            ...
            for (int j = 0; j < BOARD_LIMIT; j++) {
                ...
                var config = positionConfig.split(",");
                var expected = Integer.parseInt(config[0]);
                var fixed = Boolean.parseBoolean(config[1]);
    ...
    private static void inputNumber() {
        ...
        System.out.printf("Informe o número (1-9) para [%s,%s]: ", col, row);
    ...
    private static void showCurrentGame() {
        ...
        for (int row = 0; row < BOARD_LIMIT; row++) {
            for (int col = 0; col < BOARD_LIMIT; col++) {
                var value = board.getSpaces().get(row).get(col).getActual();
    ...
    private static void giveHint() {
        ...
        for (int row = 0; row < BOARD_LIMIT; row++) {
            for (int col = 0; col < BOARD_LIMIT; col++) {
                ...
                if (isNull(space.getActual())) {
                    System.out.printf("Dica: na posição [%s,%s] o valor correto é %d\n",
        ...
        System.out.println("Não há mais dicas disponíveis, todas as posições estão preenchidas.");
    }
}
